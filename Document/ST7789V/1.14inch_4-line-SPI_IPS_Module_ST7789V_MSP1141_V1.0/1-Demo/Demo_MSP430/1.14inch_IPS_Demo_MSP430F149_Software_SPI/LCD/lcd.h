...
#define	LCD_LED(n) (n?(P2OUT |= LED):(P2OUT &= ~LED)) //LCD背光    		 
//如果使用官方库函数定义下列底层，速度将会下降到14帧每秒，建议采用我司推荐方法
//以下IO定义直接操作寄存器，快速IO操作，刷屏速率可以达到28帧每秒！ 

//GPIO置位（拉高）
#define	LCD_RS_SET	P2OUT|=LCD_RS    //数据/命令   
#define	LCD_RST_SET	P2OUT|=LCD_RST   //复位			  
#define	LCD_CS_SET      P2OUT|=LCD_CS   //片选

//GPIO复位（拉低）							    
#define	LCD_RS_CLR	P2OUT&=~(LCD_RS)     //数据/命令 	 
#define	LCD_RST_CLR	P2OUT&=~(LCD_RST)     //复位			 
#define	LCD_CS_CLR      P2OUT&=~(LCD_CS)      //片选
//画笔颜色
#define WHITE       0xFFFF
#define BLACK      	0x0000	  
#define BLUE       	0x001F  
